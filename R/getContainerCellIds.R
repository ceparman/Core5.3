...
getContainerCellIds<-function(coreApi, containerType,containerBarcode,useVerbose = FALSE)
{
...
query   <- paste0("('",containerBarcode,"')?$expand=REV_IMPL_CONTAINER_CELL")
...
cells<-unlist(lapply(out$content$REV_IMPL_CONTAINER_CELL, function(x) x$Id))
